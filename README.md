...
The current first slice has 29 passing tests, three task types, five execution conditions, evidence projections, scoring utilities, a complete 75-run local matrix, retry-feedback and safety comparisons, duplicate-suppression results, a calibrated hosted probe, and a five-run hosted cost baseline. The next step tests a narrow hosted cost envelope.
