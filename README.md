...
The current first slice has 32 passing tests, three task types, five execution conditions, evidence projections, scoring utilities, a complete 75-run local matrix, retry-feedback and safety comparisons, duplicate-suppression results, a calibrated hosted probe, a five-run hosted cost baseline, a hosted cost-envelope observation, and one successful hosted tool trace. The next step is to establish a compact hosted-path oracle before testing a hosted failure mode.
