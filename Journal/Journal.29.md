...
This differs from the calculator failure in one important way. The two lookups are the only source of option prices for the controlled task. If both fail, the model has no tool-supplied comparison evidence. The run therefore gives the real hosted model a meaningful opportunity to produce an invalid or unavailable outcome, retry a dependency, or otherwise reveal its failure-handling behavior.
