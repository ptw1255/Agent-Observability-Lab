...
The successful baseline showed that telemetry reconstructs one efficient hosted path. A useful observability system must also describe deviations from that path. This is the smallest meaningful deviation: one reusable tool boundary fails once while every other part of the hosted task remains fixed.
...
The notable condition is not “did the harness inject an error?” We already know that. The meaningful result will be what the real model does after the error and whether the trace explains that behavior.
